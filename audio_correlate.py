...
def calculate_fingerprints(filename):
    ...
    fpcalc_out = subprocess.getoutput('fpcalc -raw -length %i %s' % (sample_time, tmpFile))
...
def get_max_corr(corr, source, target):
    ...
    if corr[max_corr_index] > threshold:
        print(('MATCH with correlation of %.4f at offset %i' % (corr[max_corr_index], max_corr_offset)))
    else:
        print(('NO MATCH (Correlation: %.4f at offset %i)' % (corr[max_corr_index], max_corr_offset)))
